...
def move_root_toward_target_xz(agent_root_prim, root_pos, target_pos, step_units):
    """
    Move AgentRoot toward the target on the XZ plane only.
    Y is preserved.
    """
...
def main():
    ...
    while True:
        ...
        distance_to_target = horizontal_distance_xz(root_pos, target_pos)

        camera_distance_to_target = horizontal_distance_xz(camera_pos, target_pos)
        ...
        if state == "patrol":
            ...
            if alignment < LOSE_TARGET_THRESHOLD:
                ...
                # Always apply best yaw step first
                set_root_yaw_deg(agent_root_prim, next_yaw)

                # Maintain best observed approach distance during this lock episode
                arrived = distance_to_target <= ARRIVAL_DISTANCE

                # Transition to arrival hold
                if arrived:
                    state = "hold_at_target"

                # Approach whenever lock is strong enough
            ...
            if alignment < LOSE_TARGET_THRESHOLD:
                ...
                # Hold orientation only. No forward motion here.
                set_root_yaw_deg(agent_root_prim, next_yaw)
                stage.Save()

                # If we drift too far away again, go back to approach mode
                if distance_to_target > HOLD_EXIT_DISTANCE:
                    state = "focus_target"
                    best_distance_to_target = distance_to_target
        ...
        print(
            ...
            f"root_dist: {distance_to_target: .2f} | "
            f"cam_dist: {camera_distance_to_target: .2f}"
        )

        print(
            f"target_pos: ({target_pos[0]:.2f}, {target_pos[1]:.2f}, {target_pos[2]:.2f}) | "
            f"root_pos: ({root_pos[0]:.2f}, {root_pos[1]:.2f}, {root_pos[2]:.2f}) | "
            f"camera_pos: ({camera_pos[0]:.2f}, {camera_pos[1]:.2f}, {camera_pos[2]:.2f})"
        )
        time.sleep(SLEEP_SEC)
