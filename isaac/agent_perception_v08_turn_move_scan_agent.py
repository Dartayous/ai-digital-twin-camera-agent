...
# Reacquire
REACQUIRE_STEP_DEG = 3.0
REACQUIRE_MAX_FRAMES = 25

# Better warehouse coverage waypoints
# Adjust later if needed, but start with these.
WAYPOINTS = [
    (0.0,   8.0,  20.0),
    (0.0,   8.0,   8.0),
    (12.0,  8.0,   8.0),
...
def desired_yaw_to_point(from_pos, to_pos):
    ...
    # IMPORTANT:
    # This convention is chosen to match the corrected "forward" motion below.
    # If later needed, this is the one place to adjust globally.
    return math.degrees(math.atan2(dx, dz))

def move_forward(prim, yaw_deg, step):
    """
    Corrected forward motion for this rig.
    This version intentionally flips the previous sign convention.
    """
...
async def main():
    ...
    while True:
        ...
        yaw_error_to_wp = wrap_angle_deg(desired_yaw - yaw)
        ...
        if state == "turn_to_waypoint":
            if usable_visible:
                ...
                if abs(yaw_error_to_wp) <= WAYPOINT_FACE_TOLERANCE_DEG:
                    state = "move_to_waypoint"
                else:
                    if yaw_error_to_wp > 0:
        ...
        # ====================================================
            ...
            if usable_visible:
                ...
                if wp_dist <= WAYPOINT_ARRIVAL_DIST:
                    ...
                    # If heading is off, stop translating and re-turn first
                    if abs(yaw_error_to_wp) > WAYPOINT_FACE_TOLERANCE_DEG:
                        state = "turn_to_waypoint"
                    else:
        ...
        # ====================================================
        # SCAN
        # full in-place scan at waypoint
        # ====================================================
        elif state == "scan":
            if usable_visible:
                state = "track"
        ...
        # ====================================================
        # INVESTIGATE
        # weak detection: stop and turn toward it
        # ====================================================
            ...
            if usable_visible:
                ...
                if bbox_error is not None:
                    if bbox_error > 0:
                        set_yaw_deg(root, yaw + REACQUIRE_STEP_DEG)
                        last_seen_dir = 1
                    elif bbox_error < 0:
                        set_yaw_deg(root, yaw - REACQUIRE_STEP_DEG)
                        last_seen_dir = -1
                ...
                if lost_target_frames > REACQUIRE_MAX_FRAMES:
                    lost_target_frames = 0
                    state = "scan"

        # ====================================================
        # TRACK
        # rotate only until centered enough
        # ====================================================
        elif state == "track":
            if not raw_visible:
                lost_target_frames += 1
        ...
        # ====================================================
        # APPROACH
        # move only when centered enough
        # ====================================================
        elif state == "approach":
            if not raw_visible:
                lost_target_frames += 1
        ...
        print(
            ...
            f"yaw: {yaw:.2f} | "
            f"yaw_to_wp_err: {yaw_error_to_wp:.2f} | "
            f"bbox: {bbox}"
        )
